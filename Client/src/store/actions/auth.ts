...
export const auth = (data: LoginModel | RegisterModel, isLogin: boolean): BaseThunkType<ActionAuthType> => async dispatch => {
    ...
    try {
        ...
        localStorage.setItem('userData', JSON.stringify({token: dataFetched.token, userId: token.sub, role}))
