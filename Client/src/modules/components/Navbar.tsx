  ...
  const navbarHandler = useCallback(() => {
    return (
      <ul className="nav navbar-nav ml-auto">
        {memoLinks.map((m) => (
          <li className="nav-item" key={`${m.name}-NB-${m.link}`} role="presentation">
            <NavLink exact className="nav-link" to={m.link}>
              {m.name}
            </NavLink>
          </li>
        ))}
  ...
  return (
    <nav className="navbar navbar-light navbar-expand-lg fixed-top bg-white clean-navbar">
      <div className="container">
        ...
        <div className={`collapse navbar-collapse ${classes.join('')}`} id="navcol-1">
          {navbarHandler()}
        </div>
      </div>
    </nav>
