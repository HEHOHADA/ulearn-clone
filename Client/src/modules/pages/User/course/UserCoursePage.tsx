...
export default () => {
    ...
    const fetchModule = useCallback(async (id) => {
            if (!course) {
                ...
                dispatch(userActions.choseItem({module: course!.modules?.find(c => c.id === parseInt(id))})
                )
            }
            setLoad(false)
        },
    ...
    return (
        <main className="page">
            <div className="container">
                <div className="row">
                    { loading || load || userLoading ? <Loader/> : module ?
                        <Module module={ module }
                                onChooseTheme={ choseItems }/> :

                        course && <Course loading={ loading || load || userLoading } course={ course }
                                          onChooseModule={ choseModules }/> }
                    <div className="col-md-8 col-xs-12">
                        <div className="container">
                            { theme && <Theme loading={ userLoading || load } theme={ theme }
