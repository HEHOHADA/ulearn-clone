...
export default () => {
    ...
    const onDeleteHandler =  useCallback(async (id: number) => {
    ...
    const filterHandler = async (sortBy: number, pagee?: number) => {
        try {
            ...
            const response = await axios.get(`${ subscriptionRequest }?page=${ pagee ?? page }&pageSize=${ 3 }&fromLevel=${ form.fromLevel }&toLevel=${ form.toLevel }&fromPrice=${ form.fromPrice }&toPrice=${ form.toPrice }&sortType=${ sort }`)
            if (response.data) {
        ...
        <div className="row">
            ...
            <div className="container pt-4">
                ...
                <div className="btn-block m-5">
                    <button className="btn btn-primary m-1" onClick={ () => filterHandler(0) }>По убыванию
                        цены
                    </button>
                    <button className="btn btn-primary m-1" onClick={ () => filterHandler(1) }>по
                        возрастанию цены
                    </button>
                    <button className="btn btn-primary m-1" onClick={ () => filterHandler(2) }>По убыванию
                        левела
                    </button>
                    <button className="btn btn-primary m-1" onClick={ () => filterHandler(3) }>по
                        возрастанию левела
                    </button>
                    ...
                    <nav>
                        <ul className="pagination">
                            <li className="page-item">
                                <button className="page-link" onClick={ async () => {
                                    setPage(page - 1)
